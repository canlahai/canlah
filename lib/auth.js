...
// Returns { ok, role?, demo? }. Pure — does NOT write to res. Callers write their own
// 401 if !ok. Demo mode bypasses auth so in-repo tests and demo flows work without
// credentials. Admin API key path is preserved for back-compat with curl debugging.
export function authCheck(req) {
  ...
  if (session?.role) return { ok: true, role: session.role };
  return { ok: false };
}
