 ...
 *  Explicit AUTH_MODE always wins. Otherwise prod (Vercel) defaults to per-user
 *  accounts (multi-party collaboration needs real identities); local/dev/preview
 *  default to the shared password. Set AUTH_MODE=shared to force shared in prod. */
export function usersAuthEnabled() {
  const mode = process.env.AUTH_MODE || (process.env.VERCEL_ENV === 'production' ? 'users' : 'shared');
  return mode === 'users';
}
