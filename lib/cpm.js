...
export function computeSchedule(tasks, { startDate, calendar } = {}) {
  ...
  return {
    ...
    // Guard projectDuration === 0 (a network of only zero-duration milestones):
    // the last occupied offset is the start day, not -1.
    projectEnd: workingDayDate(startDate, Math.max(0, projectDuration - 1), cal),
