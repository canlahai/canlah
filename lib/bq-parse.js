...
export function parseBqCsv(text) {
  ...
  // Duplicate refs would break matching — disambiguate so every ref is unique.
  // Check generated refs against ALL refs (original + already-assigned), so a
  // disambiguated "A-2" can't collide with a real "A-2" elsewhere in the BQ.
  const used = new Set();
  for (const l of lines) {
    let ref = l.ref;
    if (used.has(ref)) {
      let n = 2;
      while (used.has(`${ref}-${n}`)) n++;
      ref = `${ref}-${n}`;
    }
    used.add(ref);
    l.ref = ref;
