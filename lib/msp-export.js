...
/**
 * Serialise a COLLABORATIVE programme (programme.html activity shape) to MSPDI
 * XML that MS Project / Primavera P6 can open.
 *   { name, startDate, activities:[{id,name,section,durationDays,predecessors:[id]}],
 *     dates: { [activityId]: { start, end } } }   ← dates from the CPM schedule
 * Sections become summary tasks (when there's more than one); zero-duration
 * activities become milestones; predecessors become finish-to-start links.
 * Tasks are manually scheduled with explicit Start/Finish so the SG Mon–Sat +
 * public-holiday dates survive MS Project's default Mon–Fri recalculation.
 */
export function activitiesToMSProjectXML({ name, startDate, activities = [], dates = {} } = {}) {
  const get = (id) => dates[id] || {};
  const allStarts = activities.map((a) => get(a.id).start).filter(Boolean).sort();
  const allEnds = activities.map((a) => get(a.id).end).filter(Boolean).sort();
  const projStart = allStarts[0] || startDate;
  const projEnd = allEnds.slice(-1)[0] || projStart;

  // Group by section, preserving first-seen order.
  const order = [];
  const bySection = new Map();
  for (const a of activities) {
    const k = a.section || 'Activities';
    if (!bySection.has(k)) { bySection.set(k, []); order.push(k); }
    bySection.get(k).push(a);
  }
  const multi = order.length > 1;

  // UID allocation: reuse each activity's preserved mspUid where present (stable
  // round-trip), and hand out fresh, non-colliding UIDs for summaries + new tasks.
  const usedUids = new Set();
  for (const a of activities) { const u = Number(a.mspUid); if (Number.isInteger(u) && u > 0) usedUids.add(u); }
  let nextUid = 1;
  const allocUid = () => { while (usedUids.has(nextUid)) nextUid++; usedUids.add(nextUid); return nextUid; };
  const uidForActivity = (a) => { const u = Number(a.mspUid); return (Number.isInteger(u) && u > 0) ? u : allocUid(); };

  const rows = [];
  const idToUid = new Map();
  let id = 1;
  for (const sec of order) {
    const kids = bySection.get(sec);
    if (multi) rows.push({ kind: 'summary', name: sec, kids, uid: allocUid(), id: id++ });
    for (const a of kids) { const tu = uidForActivity(a); idToUid.set(a.id, tu); rows.push({ kind: 'task', a, uid: tu, id: id++, level: multi ? 2 : 1 }); }
  }

  const taskXml = rows.map((r) => {
    if (r.kind === 'summary') {
      const ss = r.kids.map((c) => get(c.id).start).filter(Boolean).sort()[0] || projStart;
      const ee = r.kids.map((c) => get(c.id).end).filter(Boolean).sort().slice(-1)[0] || ss;
      return `    <Task><UID>${r.uid}</UID><ID>${r.id}</ID><Name>${esc(r.name)}</Name><OutlineLevel>1</OutlineLevel><Summary>1</Summary><Manual>1</Manual><Start>${fmtStart(ss)}</Start><Finish>${fmtFinish(ee)}</Finish></Task>`;
    }
    const a = r.a;
    const d = get(a.id);
    const dur = Math.max(0, Math.round(Number(a.durationDays) || 0));
    const isMs = dur === 0;
    const preds = (a.predecessors || []).map((p) => {
      const pid = typeof p === 'string' ? p : p.id;
      const lag = typeof p === 'string' ? 0 : (Number(p.lagDays) || 0);
      const pu = idToUid.get(pid);
      return pu ? `      <PredecessorLink><PredecessorUID>${pu}</PredecessorUID><Type>1</Type><LinkLag>${lagTenths(lag)}</LinkLag><LagFormat>7</LagFormat></PredecessorLink>` : '';
    }).filter(Boolean).join('\n');
    const s = d.start || projStart, e = d.end || s;
    return `    <Task>
      <UID>${r.uid}</UID><ID>${r.id}</ID>
      <Name>${esc(a.name || a.id)}</Name>
      <OutlineLevel>${r.level}</OutlineLevel>
      <Manual>1</Manual>
      <Milestone>${isMs ? 1 : 0}</Milestone>
      <Duration>${durHours(dur)}</Duration>
      <DurationFormat>7</DurationFormat>
      <Start>${fmtStart(s)}</Start><Finish>${fmtFinish(e)}</Finish>${preds ? '\n' + preds : ''}
    </Task>`;
  }).join('\n');
...
<Project xmlns="http://schemas.microsoft.com/project">
  <SaveVersion>14</SaveVersion>
  <Name>${esc(name || 'CanLah Programme')}</Name>
  <Title>${esc(name || 'CanLah Programme')}</Title>
  <Author>CanLah.ai</Author>
  <StartDate>${fmtStart(projStart)}</StartDate>
  <FinishDate>${fmtFinish(projEnd)}</FinishDate>
  <CalendarUID>1</CalendarUID>
  <DefaultTaskType>0</DefaultTaskType>
${STANDARD_CALENDAR}
  <Tasks>
