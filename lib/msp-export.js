...
export function activitiesToMSProjectXML({ name, startDate, activities = [], dates = {} } = {}) {
  ...
  for (const sec of order) {
    ...
    if (multi) rows.push({ kind: 'summary', name: sec, kids, uid: uid++, id: id++ });
    for (const a of kids) { const tu = uid++; idToUid.set(a.id, tu); rows.push({ kind: 'task', a, uid: tu, id: id++, level: multi ? 2 : 1 }); }
  }
