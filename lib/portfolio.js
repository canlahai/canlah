...
import { makeCalendar, nextWorkingDay, workingDayDate, workingDaysBetween, _internal } from './calendar.js';
...
// A delivery is a problem if marked delayed, or it's past its needed-by date and
// not yet delivered. Mirrors programme.html's deliveryLate().
function deliveryLate(d) {
  if (!d) return false;
  if (d.status === 'delayed') return true;
  if (d.status === 'delivered') return false;
  return !!(d.neededBy && d.neededBy < today());
}
...
/**
 * "Needs attention" inbox across every programme: blocked work, overdue
 * activities, work due in the next 7 days, and late/overdue deliveries. Powers
 * the in-app notifications bell + the copyable weekly digest.
 */
export async function attention(userId) {
  const t = today();
  const soon = addDays(t, 7);
  const out = { blocked: [], overdue: [], dueSoon: [], lateDeliveries: [] };
  for (const { p, sched } of await scheduledProgrammes(userId)) {
    for (const a of p.activities || []) {
      if (a.status === 'done') continue;
      const d = sched && sched.byId.get(a.id);
      const base = {
        programmeId: p.id, programmeName: p.name, activityId: a.id, name: a.name || a.id,
        start: d ? d.start : null, end: d ? d.end : null,
        responsibleParty: a.responsibleParty || a.assignee || null,
      };
      if (a.status === 'blocked') { out.blocked.push({ ...base, reason: a.blockedReason || null }); continue; }
      if (d && d.end < t) out.overdue.push(base);
      else if (d && d.start >= t && d.start <= soon) out.dueSoon.push({ ...base, assignee: a.assignee || null });
      for (const dv of (a.deliveries || [])) {
        if (deliveryLate(dv)) out.lateDeliveries.push({ ...base, item: dv.item || '', neededBy: dv.neededBy || null, responsible: dv.responsible || null });
      }
    }
  }
  out.overdue.sort((x, y) => (x.end || '').localeCompare(y.end || ''));
  out.dueSoon.sort((x, y) => (x.start || '').localeCompare(y.start || ''));
  out.total = out.blocked.length + out.overdue.length + out.dueSoon.length + out.lateDeliveries.length;
  return out;
}

/**
 * Auto re-flow: propose a new start date for each programme so that every
 * cross-project finish-to-start link is satisfied (a successor activity starts
 * the working day AFTER its predecessor finishes). Shifts propagate through the
 * dependency chain in topological order (if A slips, B moves, then C moves).
 *
 * Internal schedules are NOT rescheduled — each programme keeps its own CPM and
 * is simply pushed forward as a whole. PROPOSAL ONLY: nothing is mutated; the
 * caller applies a proposal by updating that programme's startDate.
 */
export async function reflow(userId) {
  const cal = makeCalendar();
  const ms = (iso) => _internal.parse(iso);
  const items = await scheduledProgrammes(userId);

  // Mutable per-programme working state (startDate is what we may push forward).
  const state = new Map();
  for (const { p, sched } of items) {
    state.set(p.id, {
      p, startDate: p.startDate, activities: p.activities || [],
      byId: sched ? new Map(sched.byId) : null,
      origEnd: sched ? sched.projectEnd : null,
      projectEnd: sched ? sched.projectEnd : null,
    });
  }
  const recompute = (st) => {
    if (!st.activities.length || !st.startDate) { st.byId = null; st.projectEnd = null; return; }
    try {
      const s = computeSchedule(normaliseTasks(st.activities), { startDate: st.startDate });
      st.byId = new Map(s.tasks.map((t) => [t.id, { start: t.start, end: t.end }]));
      st.projectEnd = s.projectEnd;
    } catch { st.byId = null; st.projectEnd = null; }
  };

  // Programme-level dependency graph (predecessor programme → successor programme).
  const links = [];
  const indeg = new Map([...state.keys()].map((id) => [id, 0]));
  const succs = new Map([...state.keys()].map((id) => [id, []]));
  for (const { p } of items) {
    for (const a of p.activities || []) {
      for (const d of (Array.isArray(a.externalDeps) ? a.externalDeps : [])) {
        if (!state.has(d.programmeId) || d.programmeId === p.id) continue;
        links.push({ succProg: p.id, depAct: a.id, predProg: d.programmeId, predAct: d.activityId });
        indeg.set(p.id, indeg.get(p.id) + 1);
        succs.get(d.programmeId).push(p.id);
      }
    }
  }

  // Topological order (Kahn). Programmes left over are in a dependency cycle.
  const queue = [...state.keys()].filter((id) => indeg.get(id) === 0);
  const order = [];
  while (queue.length) {
    const id = queue.shift(); order.push(id);
    for (const s of succs.get(id)) { indeg.set(s, indeg.get(s) - 1); if (indeg.get(s) === 0) queue.push(s); }
  }
  const cyclic = [...state.keys()].filter((id) => !order.includes(id));

  // Push each programme just enough to clear all its incoming links.
  for (const pId of order) {
    const st = state.get(pId);
    if (!st.byId) continue;
    let k = 0;
    for (const l of links.filter((x) => x.succProg === pId)) {
      const q = state.get(l.predProg);
      const predFinish = q && q.byId && (q.byId.get(l.predAct) || {}).end;
      const depStart = (st.byId.get(l.depAct) || {}).start;
      if (!predFinish || !depStart) continue;
      const target = nextWorkingDay(_internal.addDays(predFinish, 1), cal); // FS: day after finish
      if (ms(depStart) < ms(target)) k = Math.max(k, workingDaysBetween(depStart, target, cal));
    }
    if (k > 0) { st.startDate = workingDayDate(st.startDate, k, cal); recompute(st); }
  }

  const proposals = [];
  for (const { p } of items) {
    const st = state.get(p.id);
    if (st.startDate !== p.startDate) {
      proposals.push({
        programmeId: p.id, programmeName: p.name,
        currentStart: p.startDate, proposedStart: st.startDate,
        shiftWorkingDays: workingDaysBetween(p.startDate, st.startDate, cal),
        currentEnd: st.origEnd, proposedEnd: st.projectEnd,
      });
    }
  }
  return { proposals, cyclic, hasCycle: cyclic.length > 0 };
}
